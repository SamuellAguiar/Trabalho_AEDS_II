...
public class GerenciarBases {
    ...
    public static void bubbleSortEmDisco(String arquivo) throws IOException {
        ...
        do {
            ...
            if (!orig.delete())
                throw new IOException("Falha ao deletar " + arquivo);
    ...
    private static int parseKey(String linha) {
        try {
            return Integer.parseInt(linha.split(";", 2)[0].trim());
        } catch (Exception e) {
    ...
    private static void swapLinhasEmDisco(String arquivo, int pos1, int pos2)
        ...
        if (!orig.delete())
            throw new IOException("Erro ao deletar original");
        if (!tmp.renameTo(orig))
            throw new IOException("Erro ao renomear swap");
    }
    ...
    private static boolean estaOrdenadoPorId(String arquivo) throws IOException {
        try (BufferedReader r = new BufferedReader(new FileReader(arquivo))) {
            ...
            while ((curr = r.readLine()) != null) {
                if (prev != null && parseKey(prev) > parseKey(curr))
                    return false;
                prev = curr;
            }
            return true;
    ...
    private static void salvarLog(String arquivo, long tempoNano) {
